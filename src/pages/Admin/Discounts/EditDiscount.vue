<template>
  <div class="mx-auto container mt-8 max-md:mt-3">
    ...
    >
      ...
      <el-form-item label="Đơn hàng tối thiểu">
...
import {createDiscount, editDiscount, getDiscountDetail} from "@/service/DiscountsServive.js";
...
const loadDiscount = async () =>{
  const result = await getDiscountDetail(route.params.id)
  if(result){
...
function onSubmit() {
  ...
  const fetchApi = async () => {
    const result = await editDiscount(route.params.id, sizeForm)
    if(result){
      ElNotification({
        title: 'Success',
